...
window.theme = {};
var menuList = window.opener.menuList;
// Navigation
(function( $ ) {
	...
	$.ajax({
		...
		success:function(list){
			...
			$.each(list, function(i, obj){
				...
				if(obj.itemType == "root") {    						
						value += "<li class='nav-active'>" +
						"<a href='" + obj.itemLink + "'>" + 
						"<i class='" + obj.itemIcon  + "' aria-hidden='true'></i>" +
						"<span>" + obj.itemName + "</span>" +
						"</a></li>"
				}else{
					var childInfo = obj.childInfo.split("^");
					//childInfo : itemLink@itemName^itemLink@itemName
					value += "<li class='nav-parent'>" +
					"<a>" +
					"<i class='" + obj.itemIcon + "' aria-hidden='true'></i>" +
					"<span>" + obj.itemName + "</span>" +
					"</a>" +
					...
					for(var i=0;i<childInfo.length;i++){
						var subMenu = childInfo[i].split("@");
						value += "<li>" +
						"<a href='" + subMenu[0] + "'>" + subMenu[1] + "</a>" +
						"</li>"    							
					}
					value += "</ul></li>"    						
				}    					
			...
			console.log("미리보기 조회용 ajax 통신 실패")
		}
	});
